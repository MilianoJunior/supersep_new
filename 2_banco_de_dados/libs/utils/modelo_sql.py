...
"""
def tratamento_codigo(nome_usina, numero_ugs):
    global codigo
    codigo = codigo.replace('NOME_USINA',nome_usina)
    codigo = codigo.replace('NUMERO_UGS',numero_ugs)
def existe_file(arquivo):
    print(f"Verificando se o arquivo {arquivo} existe...")
    return os.path.exists(arquivo)
def criar_pasta(diretorio):
    try:
        print(f"Criando pasta {diretorio}...")
        os.mkdir(diretorio)
...
def criar_modelo(nome_usina, numero_ugs, resetar=False):
    # Criando e escrevendo no arquivo
    print('---------------------------------------------------------------')
    print('_______________________________________________________________')
    print(f"Criando modelo de banco de dados para a usina {nome_usina}...")
    print('_______________________________________________________________')
    tratamento_codigo(str(nome_usina), str(numero_ugs))
    arquivo = f'libs\\usinas\\{nome_usina}\\sql_{nome_usina}.py'
    pasta = f'libs\\usinas\\{nome_usina}'
    init_path = f'libs\\usinas\\{nome_usina}\\__init__.py'
    if not existe_file(pasta):
        criar_pasta(pasta)
        criar_arquivo(arquivo)
    else:
        print(f"A pasta {pasta} já existe!")
    modulo_path = f'libs.usinas.{nome_usina}.sql_{nome_usina}'
    print('---------------------------------------------------------------')
    return modulo_path