...
control = {
    'cgh_fae local': {
        ...
        'insert sql local': {
            'Command ID': base_addr + str(LW_INSERT),
            'Row select': base_addr + str(LW_INSERT + 1),
            'Row status': base_addr + str(LW_INSERT + 2),
            'Error code': base_addr + str(LW_INSERT + 3),
            'Error message': base_addr + str(LW_INSERT + 4)
        },
        'consulta sql local': {
            'Command ID': base_addr + str(LW_CONSULT),
            'Row select': base_addr + str(LW_CONSULT + 1),
            'Row status': base_addr + str(LW_CONSULT + 2),
            'Error code': base_addr + str(LW_CONSULT + 3),
            'Error message': base_addr + str(LW_CONSULT + 4),
            'Comand ID Help': ['1 - Mensal e UG-01 e UG02', '2 - Mensal e UG-01', '3 - Mensal e UG-02',
                               '4 - Diario e UG-01 e UG02', '5 - Diario e UG-01', '6 - Diario e UG-02',
                               '7 - Horas e UG-01 e UG02', '8 - Horas e UG-01', '9 - Horas e UG-02',
                               '10 - Todos UG01', '11 - Todos UG02'],
        },
    },
}
...
//--------------------------------------------------
    ...
    int LW_control, LW_control_status, LW_INSERT, LW_query_status
    
    LW_control = {str(LW_DB + 10)}
    LW_control_status= {str(LW_DB)}
    LW_query = {str(LW_INSERT)}
    LW_query_status = {str(LW_INSERT + 3)}
    
    GetData(status,"SuperSEP",LW,LW_control_status,1)
    
	if status == 2 then
	    control = 1
		SetData(control,"SuperSEP",LW,LW_INSERT,1)
		DELAY(3000)
		GetData(error_code,"SuperSEP",LW,LW_INSERT_status,1)
...
for tags in tags_list:
    for key, value in tags.items():
        ...
        if reduz:
            ...
            name = f'{str(cont)}{name}'
            lista_reducao[key] = name
            colunas += f'{name} {tipo} DEFAULT NULL,\n '
        else:
            name = rename_columns(key)
            print(f"'{str(cont)}{name}': '{key}',")
            colunas += f'{str(cont)}{key} {tipo} DEFAULT NULL,\n '
...
#         resp = buscar_tags(value[1],'tags_fae.csv')
#         print(tabulate(resp, headers='keys', tablefmt='psql'))
#         print('_______________________________________________________________')
print('_______________________________________________________________')
print('_______________________________________________________________')
print(f"----> 6) MACRO: INSERT")
print('_______________________________________________________________')
print(macro_offline_insert)

print('_______________________________________________________________')
print('_______________________________________________________________')
print(f"----> 7) MACRO: CONSULTA")
print('_______________________________________________________________')
...
SELECT 
    YEAR(data_hora) AS `Ano`,
    MONTH(data_hora) AS `Mês`,
    ROUND(MAX(ug02_acumulador_energia) - MIN(ug02_acumulador_energia), 2) AS `UG02 Acumulado mensal (MW)`
FROM 
    cgh_fae 
WHERE
    201ae != 0
...
SELECT 
    DATE(data_hora) AS `Data`,
    ROUND(MAX(ug02_acumulador_energia) - MIN(ug02_acumulador_energia), 2) AS `UG02 Energia Diária (MW)`
FROM 
    cgh_fae 
WHERE 
    DATE(data_hora) BETWEEN '${{1}}' AND '${{2}}'
...
GROUP BY 
    DATE(data_hora);
''',
    'todos_ug01': f'''SELECT data_hora AS 'data', ROUND(ug01_nivel_agua/100, 2) AS 'nivel agua', ug01_tensao_fase_A AS 'tensao fase A', ug01_tensao_fase_B AS 'tensao fase B', ug01_tensao_fase_C AS 'tensao fase C', ug01_corrente_fase_A AS 'corrente fase A', ug01_corrente_fase_B AS 'corrente fase B', ug01_corrente_fase_C AS 'corrente fase C', ug01_tensao_excitacao AS 'tensao excitacao', ug01_corrente_excitacao AS 'corrente excitacao', ug01_frequencia AS 'frequencia', ug01_potencia_ativa AS 'potencia ativa', ug01_potencia_reativa AS 'potencia reativa', ug01_potencia_aparente AS 'potencia aparente', ROUND(ug01_fp/1000,2) AS 'fp', ug01_horimetro_eletrico AS 'horimetro eletrico', ROUND(ug01_distribuidor/10,2) AS 'distribuidor', ug01_velocidade AS 'velocidade', ROUND(ug01_acumulador_energia,2) AS 'acumulador energia', ug01_temp_enrol_A AS 'temp fase A', ug01_temp_enrol_B AS 'temp fase B', ug01_temp_enrol_C AS 'temp fase C' FROM cgh_fae WHERE data_hora >= '${{1}}' AND data_hora <= '${{2}}' ORDER BY data_hora DESC;''',

    'todos_ug02': f'''SELECT data_hora AS 'data', ROUND(ug01_nivel_agua/100, 2) AS 'nivel agua', ug02_tensao_fase_A AS 'tensao fase A', ug02_tensao_fase_B AS 'tensao fase B', ug02_tensao_fase_C AS 'tensao fase C', ug02_corrente_fase_A AS 'corrente fase A', ug02_corrente_fase_B AS 'corrente fase B', ug02_corrente_fase_C AS 'corrente fase C', ug02_tensao_excitacao AS 'tensao excitacao', ug02_corrente_excitacao AS 'corrente excitacao', ug02_frequencia AS 'frequencia', ug02_potencia_ativa AS 'potencia ativa', ug02_potencia_reativa AS 'potencia reativa', ug02_potencia_aparente AS 'potencia aparente', ROUND(ug02_fp/1000,2) AS 'fp', ug02_horimetro_eletrico AS 'horimetro eletrico', ROUND(ug02_distribuidor/10,2) AS 'distribuidor', ug02_velocidade AS 'velocidade', ROUND(ug02_acumulador_energia,2) AS 'acumulador energia', ug02_temp_enrol_A AS 'temp fase A', ug02_temp_enrol_B AS 'temp fase B', ug02_temp_enrol_C AS 'temp fase C' FROM cgh_fae WHERE data_hora >= '${{1}}' AND data_hora <= '${{2}}' ORDER BY data_hora DESC;'''
}
if reduz:
    ...
    for key, value in querys_consulta.items():
        new_query = substuir_nome_query(value)
...
'''
Endereçar as Tags do comando manual para as duas máquinas
Confirar as escalas das leituras das vibrações e set points de vibração
SELECT data_hora AS 'data', ROUND(ug02_nivel_agua/100, 2) AS 'nivel agua', 02tfA AS 'tensao fase A', 02tfB AS 'tensao fase B', 02tfC AS 'tensao fase C', 02cfA AS 'corrente fase A', 02cfB AS 'corrente fase B', 02cfC AS 'corrente fase C', 02te AS 'tensao excitacao', 02ce AS 'corrente excitacao', 02f AS 'frequencia', 02pa AS 'potencia ativa', 02pr AS 'potencia reativa', 02pa AS 'potencia aparente', ROUND(02f/1000,2) AS 'fp', 02he AS 'horimetro eletrico', ROUND(02d/10,2) AS 'distribuidor', 02v AS 'velocidade', ROUND(02ae,2) AS 'acumulador energia', 02teA AS 'temp fase A', 02teB AS 'temp fase B', 02teC AS 'temp fase C' FROM cgh_fae WHERE data_hora >= '${1}' AND data_hora <= '${2}' ORDER BY data_hora DESC;

SELECT data_hora AS `data`,
ROUND(301na/100, 2) AS `nivel agua`,
4602tfA AS `tensao fase A`,
4702tfB AS `tensao fase B`,
4802tfC AS `tensao fase C`,
4902cfA AS `corrente fase A`,
5002cfB AS `corrente fase B`,
5102cfC AS `corrente fase C`,
5202te AS `tensao excitacao`,
5302ce AS `corrente excitacao`,
5402f AS `frequencia`,
5502pa AS `potencia ativa`,
5602pr AS `potencia reativa`,
5702pa AS `potencia aparente`,
ROUND(5802f/1000,2) AS `fp`,
6202he AS `horimetro eletrico`,
ROUND(5902d/10,2) AS `distribuidor`,
6002v AS `velocidade`,
ROUND(4502ae,2) AS `acumulador energia`,
6302teA AS `temp fase A`,
6402teB AS `temp fase B`,
6502teC AS `temp fase C` FROM cgh_fae WHERE data_hora >= '${1}' AND data_hora <= '${2}' ORDER BY data_hora DESC;

SELECT data_hora AS `data`,
ROUND(301na/100, 2) AS `nivel agua`,
401tfA AS `tensao fase A`,
501tfB AS `tensao fase B`,
601tfC AS `tensao fase C`,
701cfA AS `corrente fase A`,
801cfB AS `corrente fase B`,
901cfC AS `corrente fase C`,
1001te AS `tensao excitacao`,
1101ce AS `corrente excitacao`,
1201f AS `frequencia`,
1301pa AS `potencia ativa`,
1401pr AS `potencia reativa`,
1501pa AS `potencia aparente`,
ROUND(1601f/1000,2) AS `fp`,
2001he AS `horimetro eletrico`,
ROUND(1701d/10,2) AS `distribuidor`,
1801v AS `velocidade`,
ROUND(201ae,2) AS `acumulador energia`,
2101teA AS `temp fase A`,
2201teB AS `temp fase B`,
2301teC AS `temp fase C` FROM cgh_fae WHERE data_hora >= '${1}' AND data_hora <= '${2}' ORDER BY data_hora DESC;
'''
